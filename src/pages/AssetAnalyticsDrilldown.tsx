...
export default function AssetAnalyticsDrilldown() {
  ...
  const isAssets = metric === 'total-equipment' || metric === 'total-asset-value' || metric === 'assets';
  ...
  const cols = isAssets ? 7 : isParcels ? 6 : isCrops ? 3 : isEquip ? 3 : 5;
  ...
  if (data && !isLoading && !isError) {
    if (isAssets) {
      ...
      headers = ['Crop', 'Hectares', 'Parcels'];
      rows = items.map(c => <TableRow key={c.id}><TableCell className="font-medium">{c.crop}</TableCell><TableCell>{c.hectares} ha</TableCell><TableCell>{c.parcelCount}</TableCell></TableRow>);
    } else if (isEquip) {
      headers = ['Equipment', 'Type', 'Engine Hours'];
      rows = items.map(e => <TableRow key={e.id}><TableCell className="font-medium">{e.name}</TableCell><TableCell className="capitalize">{e.type}</TableCell><TableCell>{e.engineHours} hrs</TableCell></TableRow>);
    } else if (isMaint) {

    ...
    if (existing.length) {
      await prisma.$executeRawUnsafe(
        `UPDATE public.procurement
         SET supplier = $2, supplier_id = $3::uuid, inventory_id = $4::uuid, quantity = $5, unit_price = $6,
             total_cost = $7, status = $8, expected_date = $9, received_at = $10, updated_at = NOW()
         WHERE id = $1::uuid`,
        existing[0].id,
        row.supplier,
        row.supplier_id,
